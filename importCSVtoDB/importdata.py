...
file_path = r'E:\sathu\project_db-main\project_db-main\importCSVtoDB\aihitdata-worldwide-10k.csv'
...
for _, row in data2.iterrows():
    try:
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (row['id'], row.get('name', ''), row['website'], row['url'], row.get('description_short', ''), 1))
